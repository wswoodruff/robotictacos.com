...
export async function init() {
  ...
  const plane = Primitives.plane({store:_o, scale: 0.1, color: 0xaffaff});

  plane.position.set(0,0.1,0);
  plane.scale.setScalar(0.2);
  SuperObject3D.decoSuper3D(plane);
  _o.addObject3D(plane);
  // plane.material.color.setHex(0x111111)
  // plane.material = new MeshStandardMaterial( { color: 0xffaaaa } );
  
  plane.add( new AxisHelperWithLetters({store:store, size:1, letterSize:0.2}) );

  const _driver = new Driver({store:store, object: plane, 
  	friction: 0, damping: 0.80502, speed: 0.006, tilt: Math.PI*2*2.5,
  	allowTilt: true,
  	allowTurning: false })
